...
export function CheckoutForm() {
  ...
  const onSubmit = async (values: CustomerFormValues) => {
    ...
    try {
      ...
      // Using type assertion because types.ts hasn't been regenerated yet
      const orderData = {
      ...
      const { data: order, error: orderError } = await supabase
        .from('orders' as 'products') // Type workaround until types are regenerated
      ...
      // Create order items
      const orderRecord = order as unknown as { id: string; order_number: string };
      ...
      const { error: itemsError } = await supabase
        .from('order_items' as 'products') // Type workaround
      ...
      toast({
        ...
        description: language === 'de' 
          ? `Ihre Bestellnummer: ${orderRecord.order_number}` 
          : `Your order number: ${orderRecord.order_number}`,
      });

      navigate(`/order-confirmation/${orderRecord.order_number}`);
    } catch (error) {
